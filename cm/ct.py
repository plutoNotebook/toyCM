...
class ConsistencyModel(nn.Module):
    ...
    def train_step(self, x, cond, train_step, max_steps):
        ...
        t2 = t[t_idx + 1]
        # compute the loss
        loss = self.consistency_loss(x, cond, t1, t2, train_step)
    ...
    def consistency_loss(self, x, cond, t1, t2, train_step):
        """
        ...
        if self.score_corrector:
            ...
            with torch.no_grad():
                ...
                self.deltas.append(abs(x_2 - x_2_hat).mean())
                #x_2 = (x_2_hat + x_2) / 2
                #x_2 = (1. - mu) * x_2_hat + mu * x_2
    ...
    def get_current_mu(self, train_step):
        gamma = 0.5       
        initial = 0.7     
        step = 100 
        stage = train_step // step
        ratio = 1 - (initial / (1 + gamma * stage))
        return ratio
    ...
    def sample_diffusion_euler(self, x_shape, cond, n_sampling_steps=None, return_seq=False):
        ...
        # sample the sequence of timesteps
        sigmas = self.sample_seq_timesteps(N=n_sampling_steps, type='karras')
        sampled_x.append(x)
        # iterate over the remaining timesteps
